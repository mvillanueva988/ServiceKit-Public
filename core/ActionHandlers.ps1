...
#  ActionHandlers - los handlers de las acciones individuales [A][1]-[19]
# â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
# #26 Parte B: salieron de core/Router.ps1, que era el unico god-file del repo
# (2.737 LOC, 42 funciones, 15% del codigo). El Router se quedo con lo que es:
# render de menu + dispatch. Aca viven los handlers que ese dispatch invoca.
#
# NO hay cambio de comportamiento: las funciones se movieron TAL CUAL, sin
# tocarles una linea. main.ps1 dot-sourcea todos los .ps1 de core/ a nivel de
# script (Get-ChildItem ordenado por nombre), asi que este archivo se carga solo
# sin tocar el cargador, y las funciones quedan en el MISMO scope que antes.
#
# QUE se movio: los 19 Invoke-Action* mas sus tres ayudantes propios
# (Show-OrphanModuleResult, Show-RecoveryKeysOnScreen, Invoke-EncryptionGate).
#
# QUE NO: Confirm-Action y Write-ActionAudit. Estos handlers las usan en cada
# accion, pero tambien las usa el resto del Router: se quedaron alla y se ven
# igual porque el dot-source de main.ps1 comparte el scope.
#
# POR QUE el corte era seguro: el bloque no usaba NINGUNA variable $script:
# (verificado antes de mover). La unica del Router, $script:PctkBannerProfile,
# vive fuera de este bloque y se quedo alla.
